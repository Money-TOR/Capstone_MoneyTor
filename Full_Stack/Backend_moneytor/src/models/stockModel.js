...
// Ditambahkan parameter conn agar sinkron dengan database transaction pas edit transaksi
const generateLogId = async (conn = db) => {
  ...
  const lastNum = parseInt(rows[0].id_log.substring(1), 10);
...
const createLog = async (data, conn = db) => {
  ...
  // DI SINI KUNCINYA: generateLogId harus pakai conn yang di-passing dari controller
  const id_log = await generateLogId(conn);
